...
export function configurationChecks(): RuntimeCheck[] {
  ...
  return [
    ...
    { key: "broadcast_job", label: "Worker internal", ok: !production || Boolean(process.env.INTERNAL_JOB_SECRET && process.env.INTERNAL_JOB_SECRET.length >= 32), detail: "Secret job internal minimal 32 karakter" },
    { key: "outbox_worker", label: "Worker outbox", ok: !process.env.OUTBOX_WORKER_ENABLED || ["true", "false"].includes(process.env.OUTBOX_WORKER_ENABLED.toLowerCase()), detail: production && process.env.OUTBOX_WORKER_ENABLED === "false" ? "Worker tertanam dimatikan; endpoint job harus dijadwalkan" : "Worker tertanam aktif secara default di production" },
    { key: "outbox_batch", label: "Batch outbox", ok: !process.env.OUTBOX_BATCH_SIZE || (/^\d+$/.test(process.env.OUTBOX_BATCH_SIZE) && Number(process.env.OUTBOX_BATCH_SIZE) >= 1 && Number(process.env.OUTBOX_BATCH_SIZE) <= 100), detail: "OUTBOX_BATCH_SIZE berada pada rentang 1-100" },
    { key: "outbox_lease", label: "Lease outbox", ok: !process.env.OUTBOX_LEASE_MS || (/^\d+$/.test(process.env.OUTBOX_LEASE_MS) && Number(process.env.OUTBOX_LEASE_MS) >= 30000 && Number(process.env.OUTBOX_LEASE_MS) <= 3600000), detail: "OUTBOX_LEASE_MS berada pada rentang 30000-3600000" },
    { key: "outbox_poll", label: "Polling outbox", ok: !process.env.OUTBOX_POLL_INTERVAL_MS || (/^\d+$/.test(process.env.OUTBOX_POLL_INTERVAL_MS) && Number(process.env.OUTBOX_POLL_INTERVAL_MS) >= 1000 && Number(process.env.OUTBOX_POLL_INTERVAL_MS) <= 60000), detail: "OUTBOX_POLL_INTERVAL_MS berada pada rentang 1000-60000" },
  ];
}
