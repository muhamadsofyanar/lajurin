...
export const outboxEventStatusEnum = pgEnum("outbox_event_status", ["PENDING", "PROCESSING", "RETRY", "COMPLETED", "DEAD_LETTER"]);
export const jobRunStatusEnum = pgEnum("job_run_status", ["RUNNING", "SUCCEEDED", "FAILED", "PARTIAL"]);
export const jobAttemptStatusEnum = pgEnum("job_attempt_status", ["SUCCEEDED", "FAILED", "SKIPPED"]);
...
export const outboxEvents = pgTable("outbox_events", {
  ...
  index("outbox_events_ready_idx").on(table.status, table.availableAt, table.occurredAt),
  ...
  uniqueIndex("outbox_events_deduplication_unique").on(table.eventType, table.deduplicationKey),
]);
...
export const eventConsumptions = pgTable("event_consumptions", {
  ...
  uniqueIndex("event_consumptions_event_consumer_unique").on(table.eventId, table.consumerName),
  index("event_consumptions_consumer_idx").on(table.consumerName, table.processedAt),
]);
...
export const jobAttempts = pgTable("job_attempts", {
  id: uuid("id").primaryKey().defaultRandom(),
  jobRunId: uuid("job_run_id").notNull().references(() => jobRuns.id, { onDelete: "cascade" }),
  ...
  index("job_attempts_run_idx").on(table.jobRunId, table.startedAt),
  index("job_attempts_event_idx").on(table.outboxEventId, table.startedAt),
]);

export const deadLetterEvents = pgTable("dead_letter_events", {
  id: uuid("id").primaryKey().defaultRandom(),
  outboxEventId: uuid("outbox_event_id").notNull().references(() => outboxEvents.id, { onDelete: "restrict" }),
  reason: text("reason").notNull(),
  ...
  replayedBy: uuid("replayed_by").references(() => users.id, { onDelete: "set null" }),
}, (table) => [
  uniqueIndex("dead_letter_events_outbox_unique").on(table.outboxEventId),
  index("dead_letter_events_failed_idx").on(table.failedAt),
]);
