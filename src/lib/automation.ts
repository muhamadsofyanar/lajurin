  ...
  try {
    ...
    for (const rule of rules) {
      ...
      for (const candidate of candidates) {
        ...
        let delivery = createdDelivery;
        if (!delivery) {
          [delivery] = await db.select().from(automationDeliveries).where(and(
        ...
        try {
          ...
          await db.update(automationDeliveries).set({
            status: "SENT", attemptCount: delivery.attemptCount + 1, responseCode: result.responseCode,
    ...
    if (failedDeliveries > 0) throw new Error(`AUTOMATION_DELIVERY_FAILURES:${failedDeliveries}`);
  } catch (error) {
