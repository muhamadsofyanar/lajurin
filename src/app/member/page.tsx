...
import { courses, enrollments, lessonProgress, lessons, orders } from "@/lib/schema";
...
export default async function MemberPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  const rows = await db.select({
    ...
    .groupBy(courses.id, enrollments.createdAt)
  ...
  return <main className="app-main"><div className="shell">
    <div className="page-head"><div><span className="eyebrow">Dashboard member</span><h1 className="display" style={{ marginTop: 12 }}>Halo, {user.name.split(" ")[0]}.</h1><p>Lanjutkan belajar dan terhubung dengan komunitas.</p></div>{user.role !== "MEMBER" && <Link className="btn" href={user.role === "ADMIN" ? "/admin" : "/dashboard"}>Dashboard pengelola</Link>}</div>
