...
import { courses, lessonProgress, lessons, orders, products, users } from "@/lib/schema";
...
export default async function CustomersPage({ searchParams }: { searchParams: Promise<{ product?: string; segment?: keyof typeof segmentLabels; error?: string }> }) {
  ...
  const rows = productIds.length ? await db.select({ product: products, member: users, order: orders, course: courses }).from(orders)
