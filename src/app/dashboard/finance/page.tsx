...
import { requestPayoutAction, updatePayoutAccountAction } from "@/app/actions/finance";
...
import { merchantLedgerEntries, merchantPayoutAccounts, merchantPayouts, merchantProfiles, orders, platformSettings, products } from "@/lib/schema";
...
export default async function MerchantFinancePage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  const [[profile], [account], [settings], ledger, payouts, paidOrders, balance] = await Promise.all([
  ...
  const net = paidOrders.reduce((sum, row) => sum + (row.net ?? row.amount), 0);
  ...
  return <main className="app-main"><div className="shell finance-shell"><div className="page-head"><div><span className="eyebrow">Keuangan merchant</span><h1 className="display" style={{marginTop:12}}>Saldo & pencairan</h1><p>Komisi aktif {formatFeePercent(feeBps)}. Saldo dihitung dari ledger transaksi, bukan angka yang dapat diedit langsung.</p></div></div>
    ...
    <section className="stats stats-4"><div className="stat"><span>Penjualan kotor</span><strong>{formatRupiah(gross)}</strong></div><div className="stat"><span>Komisi platform</span><strong>{formatRupiah(fees)}</strong></div><div className="stat"><span>Pendapatan bersih</span><strong>{formatRupiah(net)}</strong></div><div className="stat stat-highlight"><span>Saldo tersedia</span><strong>{formatRupiah(balance)}</strong></div></section>
    <div className="finance-grid"><section className="panel"><div className="panel-head"><div><h2>Rekening pencairan</h2><p className="muted">Rekening ini disalin ke setiap permintaan payout agar riwayat tetap konsisten.</p></div><Landmark size={20} /></div><form className="form panel-form" action={updatePayoutAccountAction}><div className="field"><label htmlFor="bankName">Nama bank</label><input className="input" id="bankName" name="bankName" defaultValue={account?.bankName ?? ""} required maxLength={80} placeholder="Contoh: BCA" /></div><div className="field"><label htmlFor="accountNumber">Nomor rekening</label><input className="input" id="accountNumber" name="accountNumber" defaultValue={account?.accountNumber ?? ""} required inputMode="numeric" pattern="[0-9]{6,30}" maxLength={30} /></div><div className="field"><label htmlFor="accountHolder">Nama pemilik rekening</label><input className="input" id="accountHolder" name="accountHolder" defaultValue={account?.accountHolder ?? ""} required maxLength={100} /></div><button className="btn btn-primary" type="submit">Simpan rekening</button></form></section>
