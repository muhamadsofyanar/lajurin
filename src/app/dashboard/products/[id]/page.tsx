...
export default async function EditProductPage({ params, searchParams }: {
  ...
  return <main className="app-main"><div className="shell">
    <div className="page-head"><div><h1 className="display">{product.name}</h1><p>/p/{product.slug}</p></div><div className="actions"><Link className="btn" href={`/dashboard/products/${product.id}/landing`}>Landing page</Link><Link className="btn" href={`/dashboard/products/${product.id}/funnel`}>Kupon & funnel</Link>{product.status === "PUBLISHED" && <Link className="btn" href={`/p/${product.slug}`} target="_blank">Lihat halaman <ExternalLink size={15} /></Link>}<form action={togglePublishAction.bind(null, product.id)}><button className={`btn ${product.status === "PUBLISHED" ? "btn-danger" : "btn-lime"}`} type="submit">{product.status === "PUBLISHED" ? "Jadikan draf" : "Terbitkan"}</button></form></div></div>
