...
import { merchantManualPaymentAccounts, merchantProfiles, productFunnels, productLandingPages, products, productVariants, users } from "@/lib/schema";

type CheckoutSearch = { error?: string; coupon?: string; ref?: string; utm_source?: string; utm_medium?: string; utm_campaign?: string };
...
export default async function CheckoutPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<CheckoutSearch> }) {
  ...
  const [[funnel], variants] = await Promise.all([
    db.select().from(productFunnels).where(and(eq(productFunnels.productId, product.product.id), eq(productFunnels.isActive, true))).limit(1),
    db.select().from(productVariants).where(and(eq(productVariants.productId, product.product.id), eq(productVariants.isActive, true), or(isNull(productVariants.stock), gt(productVariants.stock, 0)))).orderBy(asc(productVariants.position)),
  ]);
  ...
  const deliveryLabel = product.product.type === "COURSE"
    ? "Akses kelas terbuka setelah pembayaran"
    : product.product.type === "DIGITAL"
      ? "File privat tersedia setelah pembayaran"
      : "Portal layanan terbuka setelah pembayaran";
  const checkoutTitle = product.product.type === "SERVICE" ? "Mulai pemesanan" : "Selesaikan pesanan";
  const trackingFields = <><input type="hidden" name="utmSource" value={search.utm_source ?? ""} /><input type="hidden" name="utmMedium" value={search.utm_medium ?? ""} /><input type="hidden" name="utmCampaign" value={search.utm_campaign ?? ""} /><input type="hidden" name="affiliateCode" value={search.ref ?? ""} /></>;

  return <><ConversionTracker productId={product.product.id} providerEvent="InitiateCheckout" value={discountedPrice} facebookPixelId={product.landing?.facebookPixelId} tiktokPixelId={product.landing?.tiktokPixelId} /><main className="checkout-wrap"><aside className="checkout-summary"><Brand inverse /><div className="checkout-summary-main">{product.landing?.coverImageUrl && <img className="checkout-summary-cover" src={product.landing.coverImageUrl} alt={`Cover ${product.product.name}`} />}<span className="eyebrow">Checkout aman · {product.merchantBrand ?? product.merchantName}</span><h1 className="display">{product.product.name}</h1><p>{product.product.headline}</p><div className="checkout-price-line">{discount > 0 && <del>{formatRupiah(product.product.price)}</del>}<strong>{formatRupiah(discountedPrice)}</strong></div>{coupon && <span className="coupon-applied"><BadgePercent size={16} /> Kupon {coupon.code} menghemat {formatRupiah(discount)}</span>}<div className="checkout-trust"><span><CheckCircle2 size={15} /> {deliveryLabel}</span><span><CheckCircle2 size={15} /> Transaksi tercatat di akun</span></div></div><small><LockKeyhole size={14} style={{display:"inline",verticalAlign:"middle"}} /> Payment gateway atau transfer manual</small></aside>

    <section className="checkout-main"><div className="checkout-form-card"><Link className="muted" href={`/p/${product.product.slug}`}>← Kembali ke produk</Link><h2 className="display">{checkoutTitle}</h2><p>Pesanan diproses oleh <strong>{product.merchantBrand ?? product.merchantName}</strong>. Data Anda digunakan untuk menyimpan pembayaran dan akses produk secara aman.</p>{search.error && <p className="alert">{search.error}</p>}{search.coupon && !coupon && <p className="alert">Kupon tidak valid, sudah berakhir, atau kuotanya habis.</p>}

      <form className="coupon-form" method="get"><div className="field"><label htmlFor="coupon">Punya kupon?</label><div className="input-action"><input className="input" id="coupon" name="coupon" defaultValue={search.coupon ?? ""} placeholder="Masukkan kode" /><button className="btn" type="submit">Terapkan</button></div></div><input type="hidden" name="ref" value={search.ref ?? ""} /><input type="hidden" name="utm_source" value={search.utm_source ?? ""} /><input type="hidden" name="utm_medium" value={search.utm_medium ?? ""} /><input type="hidden" name="utm_campaign" value={search.utm_campaign ?? ""} /></form>
      ...
      <form className="form" action={action}><input type="hidden" name="couponCode" value={coupon?.code ?? ""} />{trackingFields}<div className="field"><label htmlFor="name">Nama lengkap</label><input className="input" id="name" name="name" required minLength={2} autoComplete="name" /></div><div className="field"><label htmlFor="email">Email</label><input className="input" id="email" name="email" type="email" required autoComplete="email" /></div><div className="field"><label htmlFor="phone">Nomor WhatsApp</label><input className="input" id="phone" name="phone" type="tel" required minLength={9} maxLength={20} inputMode="tel" autoComplete="tel" placeholder="Contoh: 081234567890" /></div><div className="field"><label htmlFor="password">Password akun member</label><input className="input" id="password" name="password" type="password" minLength={8} required autoComplete="current-password" /><small className="muted">Jika email sudah terdaftar, masukkan password akun tersebut.</small></div>
        {variants.length > 0 && <div className="field"><label htmlFor="variantId">Pilih paket</label><select className="input" id="variantId" name="variantId" required>{variants.map((variant) => <option value={variant.id} key={variant.id}>{variant.name} — {formatRupiah(variant.price)}{variant.stock !== null ? ` · tersisa ${variant.stock}` : ""}</option>)}</select></div>}
