...
export default async function ManualPaymentPage({ params, searchParams }: { params: Promise<{ orderId: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  const directToMerchant = row.order.settlementMode === "MERCHANT_DIRECT";
  const destinationBank = row.order.manualDestinationBank || process.env.MANUAL_BANK_NAME || "BCA";
  const destinationAccount = row.order.manualDestinationAccount || process.env.MANUAL_BANK_ACCOUNT || "1234567890";
  const destinationHolder = row.order.manualDestinationHolder || process.env.MANUAL_BANK_HOLDER || "PT Rizqhub Indonesia";
  const action = submitManualPaymentAction.bind(null, row.order.id);
  return <main className="auth-wrap"><aside className="auth-aside"><Brand inverse /><div><span className="eyebrow">Transfer manual</span><h1 className="display" style={{marginTop:18}}>{row.productName}</h1><div className="price" style={{color:"var(--lime)"}}>{formatRupiah(row.order.amount)}</div><div className="bank-card"><small>Transfer tepat ke {directToMerchant ? "rekening merchant" : "rekening platform"}</small><strong>{destinationBank} · {destinationAccount}</strong><span>a.n. {destinationHolder}</span></div></div><small>Simpan bukti transfer untuk proses verifikasi.</small></aside><section className="auth-main"><div className="auth-card"><Link className="muted" href="/member/orders">← Pesanan saya</Link><h2 className="display" style={{marginTop:24}}>Konfirmasi pembayaran</h2><p>Unggah bukti setelah transfer. {directToMerchant ? "Merchant" : "Admin Rizqhub"} akan meninjau secara manual.</p>{error && <p className="alert">{error}</p>}<form className="form" action={action}><div className="field"><label htmlFor="bankName">Bank pengirim</label><input className="input" id="bankName" name="bankName" required placeholder="Contoh: BRI" /></div><div className="field"><label htmlFor="accountName">Nama pemilik rekening</label><input className="input" id="accountName" name="accountName" required /></div><div className="field"><label htmlFor="proof">Bukti transfer</label><input className="input file-input" id="proof" name="proof" type="file" required accept="image/jpeg,image/png,image/webp,application/pdf" /><small className="muted">JPG, PNG, WebP, atau PDF. Maksimal 3 MB.</small></div><div className="field"><label htmlFor="note">Catatan (opsional)</label><textarea className="input" id="note" name="note" maxLength={500} /></div><button className="btn btn-primary" type="submit">Kirim bukti pembayaran</button></form></div></section></main>;
}
