...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  return [
    ...
    ...["tentang", "filosofi", "manifesto", "trust", "merchant-code", "prohibited-products"].map((path) => ({ url: `${base}/${path}`, changeFrequency: "monthly" as const, priority: 0.7 })),
    ...rows.map((row) => ({ url: `${base}/p/${row.slug}`, lastModified: row.updatedAt, changeFrequency: "weekly" as const, priority: 0.8 })),
  ];
}
