...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><Nav /><main className="store-hero"><div className="shell store-grid"><section><span className="eyebrow">Kursus digital · oleh {row.merchantName}</span><h1 className="display">{product.headline}</h1><p className="lead">{product.description}</p><div className="trust-row" style={{justifyContent:"flex-start"}}><span><CheckCircle2 size={15} style={{display:"inline",verticalAlign:"middle"}} /> Akses setelah pembayaran</span><span><CheckCircle2 size={15} style={{display:"inline",verticalAlign:"middle"}} /> Belajar sesuai ritme</span></div><div className="course-list"><h2 style={{marginTop:32}}>Yang akan dipelajari</h2>{courseLessons.map((lesson) => <div className="course-item" key={lesson.id}><span className="course-number">{lesson.position}</span><div><strong>{lesson.title}</strong><p className="muted" style={{margin:"5px 0 0",fontSize:13}}>Materi tersedia setelah pembayaran.</p></div></div>)}</div></section><aside className="checkout-card"><span className="badge badge-live">Akses penuh</span><div className="price">{formatRupiah(product.price)}</div><p className="muted" style={{lineHeight:1.65}}>Sekali bayar untuk seluruh materi yang tersedia.</p><Link className="btn btn-primary" href={`/checkout/${product.slug}`} style={{width:"100%",marginTop:12}}>Dapatkan akses <ArrowRight size={17} /></Link><p className="muted" style={{textAlign:"center",fontSize:11,marginBottom:0}}>Pembayaran aman diproses oleh Xendit.</p></aside></div></main></>;
}
