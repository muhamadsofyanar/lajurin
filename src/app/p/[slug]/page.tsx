  ...
  return <><Nav /><ConversionTracker productId={product.id} utmSource={search.utm_source} utmMedium={search.utm_medium} utmCampaign={search.utm_campaign} facebookPixelId={landing?.facebookPixelId} tiktokPixelId={landing?.tiktokPixelId} />
    <main className={`sales-page template-${template}`} style={pageStyle}><style>{sectionStyles}</style>
      <section className="sales-hero"><div className="shell sales-hero-grid"><div className="sales-copy"><span className="sales-eyebrow"><Users size={16} /> {landing?.eyebrow || "Kursus digital"}</span><h1 className="display">{landing?.heroTitle || product.headline}</h1><p className="sales-lead">{landing?.heroSubtitle || product.description}</p>
        ...
        <Link className="btn sales-primary-cta" href={checkoutHref}>{landing?.ctaText || "Mulai belajar"} <ArrowRight size={19} /></Link><p className="sales-safe"><ShieldCheck size={15} /> Pembayaran aman · akses tercatat di akun member</p>
      </div><div className="sales-media">{landing?.heroVideoUrl ? <VideoPlayer url={landing.heroVideoUrl} title={product.name} /> : landing?.coverImageUrl ? <img src={landing.coverImageUrl} alt={`Sampul ${product.name}`} /> : <div className="sales-media-placeholder"><PlayCircle size={54} /><strong>{product.name}</strong><span>Tambahkan cover atau video melalui editor landing page.</span></div>}<div className="floating-proof rating"><Star size={19} fill="currentColor" /><span><strong>Materi terstruktur</strong><small>{courseLessons.length} materi siap dipelajari</small></span></div><div className="floating-proof access"><Award size={19} /><span><strong>Akses kelas</strong><small>Belajar lewat member area</small></span></div></div></div></section>

      <section className="shell proof-strip"><div><ShieldCheck /><span><strong>Pembayaran tercatat</strong><small>Riwayat tersimpan di akun</small></span></div><div><PlayCircle /><span><strong>{courseLessons.length} materi</strong><small>Belajar sesuai ritme Anda</small></span></div><div><Users /><span><strong>Komunitas belajar</strong><small>Terhubung dengan member</small></span></div></section>
